...
public test bool shouldReturnNonFinalSingleParameter() {
	...
	nonLocals = retrieveNonLocals(vars);
	var = findByName(vars, "param");
	
	return size(nonLocals) == 1 && !var.isFinal && !var.isLocal;
}
...
public test bool shouldReturnCorrectParamsWithLastOneFinal() {
	...
	return size(nonLocals) == 3 &&
		!param.isFinal && !param.isLocal &&
		!strParam.isFinal && !strParam.isLocal &&
		finalLastParam.isFinal && !finalLastParam.isLocal;
}
...
public test bool shouldReturnCorrectParamsWithLastOneNonFinal() {
	...
	return size(nonLocals) == 3 &&
		!param.isFinal && !param.isLocal &&
		!strParam.isFinal && !strParam.isLocal &&
		!nonFinalLastParam.isFinal && !nonFinalLastParam.isLocal;
}
